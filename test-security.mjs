...
const treeRenderer = fs.readFileSync(new URL('./dashboard/tree-renderer.js', import.meta.url), 'utf8');
...
assert.match(treeRenderer, /createElementNS\(/, 'dashboard SVG data rendering must use DOM construction');
assert.match(dashboard, /textContent/, 'dashboard data labels must use textContent');
assert.match(treeRenderer, /svg\.replaceChildren\(/, 'dashboard tree rendering must replace SVG children safely');
...
assert.doesNotMatch(treeRenderer, /\.innerHTML\s*=/, 'tree SVG should use safe DOM construction');
assert.doesNotMatch(content, /\.innerHTML\s*=/, 'the companion icon must work without an HTML/Trusted Types sink');
assert.match(treeRenderer, /tree-bole/, 'the rendered tree should include a structural bole');
assert.match(treeRenderer, /empty-trunk/, 'the empty garden should include an open trunk');
...
assert.match(worker, /safeOriginUrl\(value\)\s*\{\s*return safeSessionUrl\(value\) \|\| 'chrome:\/\/newtab';\s*\}/, 'persisted origins must use the current-extension-aware URL helper');
assert.match(worker, /function safeNavigationUrl\(value\)\s*\{\s*return safeSessionUrl\(value\);\s*\}/, 'Go Home navigation must use the current-extension-aware URL helper');
