...
The service worker treats runtime messages and content-script payloads as untrusted inputs. It rejects malformed message shapes, validates sender-tab metadata and HTTP(S) URLs, ignores synthetic page-dispatched clicks, serializes storage mutations, bounds pending relationships, keys target-blank relationships by source tab plus destination, detaches all tab aliases when a path is composted, and navigates Go Home back to the stored safe origin without closing tabs or windows. The companion now owns its static CSS inside a closed ShadowRoot; no companion stylesheet is exposed as a web-accessible resource.
